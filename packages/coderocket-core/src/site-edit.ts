import { z } from 'zod'
import { siteConnectionCapabilitySchema, siteConnectionProviderSchema } from './site-connection'
...
const requestConnectionSchema = z.object({
  ...
  provider: siteConnectionProviderSchema.refine(provider =>
    ['coderocket_data', 'stripe', 'calendly'].includes(provider)
  ),
  capability: siteConnectionCapabilitySchema.refine(capability =>
    ['data', 'payments', 'bookings'].includes(capability)
  ),
...
export function applySiteEditPlan(document: SiteDocument, plan: SiteEditPlan): SiteDocument {
  ...
  for (const operation of plan.operations) {
    if (operation.type === 'request_connection') continue
    if (operation.type === 'update_identity') {
      nextDocument.identity.name = operation.name
      nextDocument.pages = nextDocument.pages?.map(page => ({
        ...page,
        identity: page.identity ? { ...page.identity, name: operation.name } : undefined
      }))
      continue
    }
    if (operation.type === 'update_theme') {
      nextDocument.theme = updateThemeColors(nextDocument.theme, operation)
      nextDocument.pages = nextDocument.pages?.map(page => ({
        ...page,
        theme: page.theme ? updateThemeColors(page.theme, operation) : undefined
      }))
      continue
    }
...
/** Apply global prompt colours without discarding a captured page's visual measurements. */
function updateThemeColors(
  theme: SiteDocument['theme'],
  operation: z.infer<typeof updateThemeSchema>
): SiteDocument['theme'] {
  return {
    ...theme,
    ...(operation.accentColor
      ? { accentColor: safeEditableColor(operation.accentColor, theme.accentColor) }
      : {}),
    ...(operation.backgroundColor
      ? { backgroundColor: safeEditableColor(operation.backgroundColor, theme.backgroundColor) }
      : {}),
    ...(operation.foregroundColor
      ? { foregroundColor: safeEditableColor(operation.foregroundColor, theme.foregroundColor) }
      : {})
  }
}
